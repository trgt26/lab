B = 8                   # if change Bit the Change here 
...
def ecb_en(text,key,bit=B):
    ...
    for i in range(0,len(text),bit):
        block = text[i:i+bit]
        pre = x_or_str(key,pre)
        en = x_or_str(pre,block)
        result += en
    return result

def ecb_de(text,key,bit=B):
    iv = '01010101'                 # if change Bit the Change it also
    pre = iv
    result = ''
    for i in range(0,len(text),bit):
        block = text[i:i+bit]
        pre = x_or_str(pre,key)
        en = x_or_str(pre,block)
        result += en
...
key = 'S'                           # if change Bit the Change it also
